...
class CRCMarkup
{
	...
	static private function PIL_Replace( $matches )
	{
		...
		switch( $matches[ 1 ] )
		{
			case 'home': $strRef = CreateHREF( PAGE_HOME );
				break;
			case 'contact': $strRef = CreateHREF( PAGE_CONTACT );
				break;
			case 'login': $strRef = CreateHREF( PAGE_LOGIN );
				break;
			case 'news': $strRef = CreateHREF( PAGE_NEWS , 'archive' );
				break;
			case 'pages': $strRef = CreateHREF( PAGE_PAGE );
				break;
	...
	static private function FormImageTag( $Info , $Parms )
	{
		...
		return sprintf( '<div class="%s" style="%s%s">%s<img src="%s%s" style="width:100%%"/>%s%s</div>' , $sBlockType , $sSizeString , $sPoseString , $sLinkStart , $Info[ 'url' ] , $bUseThumb ? '.thumb.jpg' : '' , $sLinkEnd , $sCaptionBlock );
	}
	...
	static private function PBT_Replace( $matches )
	{
		...
		$sLink = sprintf( preg_replace( '/{{slug}}/' , $matches[ 1 ] , RCSettings_GetSetting( 'txtBlogLink' ) ) );
